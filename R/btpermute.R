#' Variable selection with pair comparisons
#' 
...
btpermute <- function(contests = NULL,
  ...
  set.seed(seed)
  seeds <- as.integer(runif(iterations, 1, 99999))
  ...
  for (l in seq_len(nvar)){
    ...
    # stores the variable that caused greates deviance reduction, for each iteration in the loop
    
    d.Var <- character(length = iterations) 
    ...
    for (j in seq_len(iterations)){
      ...
      for (k in ((length(varIndice)+1):length(allVars))[1:length(varIndice)]) { 
        ...
        if (length(Covariates) > 0) {
          updatedFormula <- stats::formula(paste(
            c(BTmformulaPV[[k - length(varIndice)]], c(BTmformula[match(Covariates, allVars)])),
            collapse="+"))
          # when there is already on Covariate and we are adding to that ((if length(Covariates)!=0)))
          devsPerIteration[[j]][k] <- stats::deviance( 
            BradleyTerry2::BTm(
              outcome = cbind(Win1,Win2),
              player1 = Item1,
      ...
      d.Var[j] <- names(devsPerIteration[[j]])[devsPerIteration[[j]] %in% min(devsPerIteration[[j]])] 
    }
