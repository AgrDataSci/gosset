...
The `worth_map()` function can be used to visually assess and compare variety performance based on different characteristics. The values represented in a worth_map are *log-worth* estimates. From the breeder or product developer perspective the function `worth_map()` offers a visualization tool to help in identifying variety performance based on different characteristics and select crossing materials.
